...
from carla_simulation.msg import LaneCurve
...
class LaneDetection(Node):
    ...
    def __init__(self):
        ...
        # Create Publishers
        lane_curve_topic = self.get_parameter('topic.lane_curve').get_parameter_value().string_value
        self.lane_curve_pub = self.create_publisher(LaneCurve,lane_curve_topic,qos_profile)
    ...
    def nodeParams(self):
        ...
        self.lane_curve_msg = LaneCurve()
        self.lane_curve_msg.header.frame_id = self.get_parameter('frame_id.lane_curve').get_parameter_value().string_value
    ...
    def imgCallback(self,rgb_msg,seg_msg):
        ...
        self.lane_curve_msg.header.stamp = seg_msg.header.stamp
        self.lane_curve_pub.publish(self.lane_curve_msg)
        if self.display:
            self.viewOutput(rgb_msg)
    ...
    def findLanes(self,img):
        ...
        if len(leftx) > 0 and len(rightx) > 0:
            ...
            self.lane_curve_msg.centre_radius = (left_radius + right_radius) / 2
            ...
            self.lane_curve_msg.centre_offset = (np.abs(self.width / 2) - np.abs(center_lane)) * self.xm_per_pix
        else:
            self.sliding_window = True
            self.draw_area = False
            self.lane_curve_msg.centre_radius = 0.0
            self.lane_curve_msg.centre_offset = 0.0

    def viewOutput(self,img_msg):
        ...
        if self.draw_area:
            ...
            cv2.putText(img,'Centre offset: ' + str(round(self.lane_curve_msg.centre_offset,4)) + ' m',(10,50),
